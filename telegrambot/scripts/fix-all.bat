...
:: 2. Check if .env file exists in config folder
if not exist ..\config\.env (
    echo WARNING: .env file not found in the config directory.
    echo Creating a template .env file. Please update it with your credentials.
    if not exist ..\config (
        mkdir ..\config
    )
    echo BOT_TOKEN=YOUR_BOT_TOKEN_HERE> ..\config\.env
    echo MONGODB_URI=YOUR_MONGODB_URI_HERE>> ..\config\.env
    echo ENCRYPTION_KEY=YOUR_ENCRYPTION_KEY_HERE>> ..\config\.env
    echo HELIUS_API_KEY=YOUR_HELIUS_API_KEY_HERE>> ..\config\.env
